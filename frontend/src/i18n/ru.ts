const ru = {
  // Боковая панель / разметка
  nav: {
    dashboard: 'Обзор',
    projects: 'Проекты',
    adminPanel: 'Администрирование',
    activity: 'Активность',
    subtitle: 'Инженерная панель',
    signOut: 'Выйти',
    administrator: 'Администратор',
    engineer: 'Инженер',
  },

  // Вход
  login: {
    heading: 'Вход',
    username: 'Имя пользователя',
    password: 'Пароль',
    submit: 'Войти',
    submitting: 'Вход\u2026',
    hint: 'По умолчанию: {{user}} / {{pass}}',
    errorWrongCreds: 'Неверное имя пользователя или пароль.',
    errorInvalid: 'Неверный ввод \u2014 проверьте поля.',
    errorNetwork: 'Сервер недоступен. Запущен ли uvicorn на порту 8000?',
    errorGeneric: 'Ошибка входа. Проверьте, что сервер запущен.',
  },

  // Панель управления
  dashboard: {
    title: 'Панель управления',
    subtitle: 'Обзор всех проектов, метрик и активности команды',
    loading: 'Загрузка\u2026',
    totalProjects: 'Всего проектов',
    activeProjects: 'Активных проектов',
    completed: 'Завершено',
    totalFiles: 'Всего файлов',
    teamMembers: 'Участников',
    avgProgress: 'Средний прогресс',
    byType: 'Проекты по типу',
    byRegion: 'Проекты по региону',
    monthlyActivity: 'Активность по месяцам',
    emptyScoped: 'Для вашей учётной записи пока нет доступных проектов.',
    activeProgress: 'Прогресс активных проектов',
  },

  // Сокращения месяцев
  months: {
    Jan: 'Янв', Feb: 'Фев', Mar: 'Мар', Apr: 'Апр',
    May: 'Май', Jun: 'Июн', Jul: 'Июл', Aug: 'Авг',
    Sep: 'Сен', Oct: 'Окт', Nov: 'Ноя', Dec: 'Дек',
  },

  // Страница проектов
  projects: {
    title: 'Проекты',
    subtitle: 'Нажмите карандаш для редактирования, скрепку — для файлов',
    newProject: 'Новый проект',
    total: '{{count}} всего',
    colCode: 'Код',
    colTitle: 'Название',
    colType: 'Тип',
    colRegion: 'Регион',
    colStatus: 'Статус',
    colProgress: 'Прогресс',
    colLead: 'Ответственный',
    colFiles: 'Файлы',
    statusActive: 'Активный',
    statusCompleted: 'Завершён',
    statusDraft: 'Черновик',
    confirmDelete: 'Удалить проект "{{code}} \u2014 {{title}}"? Это нельзя отменить.',
  },

  // Форма проекта
  projectForm: {
    titleNew: 'Новый проект',
    titleEdit: 'Редактировать проект',
    fieldCode: 'Код *',
    fieldRegion: 'Регион *',
    fieldTitle: 'Название *',
    fieldType: 'Тип *',
    fieldStatus: 'Статус',
    fieldProgress: 'Прогресс \u2014 {{value}}%',
    fieldPlannedObjects: 'Плановые объекты',
    fieldDueDate: 'Срок сдачи',
    fieldLead: 'Ответственный',
    noLead: '— Не назначен —',
    fieldDescription: 'Описание',
    placeholderType: 'Проект / ТУ / \u2026',
    placeholderNotes: 'Примечания\u2026',
    statusActive: 'Активный',
    statusDraft: 'Черновик',
    statusCompleted: 'Завершён',
    cancel: 'Отмена',
    saving: 'Сохранение\u2026',
    saveChanges: 'Сохранить',
    createProject: 'Создать проект',
    errorSave: 'Ошибка сохранения. Проверьте поля и попробуйте снова.',
  },

  // Менеджер файлов
  fileManager: {
    title: 'Файлы',
    uploading: 'Загрузка\u2026',
    clickToUpload: 'Нажмите для загрузки файла',
    noFiles: 'Файлов пока нет.',
    errorUpload: 'Ошибка загрузки. Попробуйте снова.',
    errorDelete: 'Ошибка удаления.',
    errorForbidden: 'Этот файл нельзя удалить: он загружен другим пользователем. Обратитесь к администратору.',
  },

  // Страница проекта
  detail: {
    loading: 'Загрузка проекта\u2026',
    notFound: 'Проект не найден.',
    root: 'Корень',
    newFolder: 'Новая папка',
    upload: 'Загрузить',
    save: 'Сохранить',
    folderNamePlaceholder: 'Название папки',
    fileCount: '{{count}} файл(а)',
    noFiles: 'Файлов пока нет. Загрузите выше.',
    deleteFolder: 'Удалить папку',
    confirmDeleteFolder: 'Удалить папку "{{name}}"? Это нельзя отменить.',
    folderNotEmpty: 'В папке {{count}} файл(а). Сначала удалите их.',
    folderError: 'Не удалось создать папку.',
    filesTab: 'Файлы',
    dashboardTab: 'Панель',
    kpiError: 'Не удалось загрузить KPI проекта.',
    kpiTotalFiles: 'Всего файлов',
    kpiTotalFolders: 'Всего папок',
    kpiPlannedObjects: 'Плановые объекты',
    kpiObjectFolders: 'Создано папок объектов',
    kpiMissingObjectFolders: 'Недостающие папки объектов',
    kpiAverageObjectProgress: 'Средний прогресс объектов',
    kpiProjectProgress: 'Прогресс проекта',
    objectFolder: 'Объект',
    regularFolder: 'Обычная',
    objectFolderShort: 'Объект',
    folderProgress: 'Прогресс папки',
    folderUpdateError: 'Не удалось обновить папку.',
    download: 'Скачать',
  },

  // Журнал активности
  activity: {
    title: 'Активность',
    subtitle: 'Последние действия всех участников команды',
    loading: 'Загрузка\u2026',
    empty: 'Действий пока нет.',
    refresh: 'Обновить',
    actions: {
      'file.upload':    'загрузил(а) {{entity}} в {{context}}',
      'file.delete':    'удалил(а) {{entity}} из {{context}}',
      'project.create': 'создал(а) проект {{entity}}',
      'project.update': 'обновил(а) проект {{entity}}',
      'project.delete': 'удалил(а) проект {{entity}}',
      'user.create':    'создал(а) пользователя {{entity}}',
      'user.update':    'обновил(а) пользователя {{entity}}',
      'user.delete':    'удалил(а) пользователя {{entity}}',
    },
  },

  // Администрирование
  admin: {
    title: 'Панель администратора',
    subtitle: 'Управление учётными записями и правами доступа',
    newUser: 'Новый пользователь',
    createUserTitle: 'Создать пользователя',
    fieldFullName: 'Полное имя',
    fieldUsername: 'Имя пользователя',
    fieldPassword: 'Пароль',
    fieldProjectAccess: 'Доступ к проектам',
    projectAccessHint: 'Удерживайте Ctrl/Cmd, чтобы выбрать несколько проектов.',
    errorProjectAccess: 'Не удалось загрузить доступ к проектам.',
    fieldRole: 'Роль',
    roleAdmin: 'Администратор',
    roleClient: 'Клиент',
    roleEngineer: 'Инженер',
    colUser: 'Пользователь',
    colUsername: 'Логин',
    colRole: 'Роль',
    colStatus: 'Статус',
    colJoined: 'Дата регистрации',
    colActions: 'Действия',
    statusActive: 'Активен',
    statusInactive: 'Неактивен',
    labelYou: 'вы',
    total: '{{count}} всего',
    loading: 'Загрузка\u2026',
    creating: 'Создание\u2026',
    createUser: 'Создать',
    cancel: 'Отмена',
    errorCreate: 'Не удалось создать пользователя.',
    tipPromote: 'Повысить до администратора',
    tipDemote: 'Понизить до инженера',
    tipDeactivate: 'Деактивировать',
    tipActivate: 'Активировать',
    tipDelete: 'Удалить пользователя',
    tipEdit: 'Редактировать пользователя',
    confirmDelete: 'Удалить пользователя "{{username}}"? Это нельзя отменить.',
    placeholderFullName: 'Иван Иванов',
    placeholderPassword: 'минимум 4 символа',
    editUserTitle: 'Редактировать пользователя',
    fieldNewPassword: 'Новый пароль',
    placeholderNewPassword: 'оставьте пустым, чтобы не менять',
    saving: 'Сохранение…',
    saveChanges: 'Сохранить',
  },
}

export default ru
